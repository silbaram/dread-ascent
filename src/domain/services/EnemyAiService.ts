...
export class EnemyAiService {
    ...
    decide(enemy: Enemy, playerPosition: Position): EnemyAction {
        if (this.canSeePlayer(enemy.position, playerPosition)) {
            ...
            if (this.isAdjacent(enemy.position, playerPosition)) {
    ...
    private isAdjacent(origin: Position, target: Position) {
        return Math.abs(origin.x - target.x) + Math.abs(origin.y - target.y) === 1;
    }
