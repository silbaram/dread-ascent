...
## 구현된 컴포넌트 (Sprint 001-002)

### `domain/entities/Player`

플레이어의 그리드 위치를 관리하는 순수 도메인 엔티티.

- 위치와 함께 전투 스탯(`health`, `maxHealth`, `attack`, `defense`) 및 누적 경험치(`experience`)를 유지한다
...
3. `EnemySpawnerService` 결과를 적 스프라이트와 턴 큐 actor로 반영
4. 플레이어가 적 방향으로 이동하면 `CombatService`를 통해 범프 공격을 수행
5. 플레이어 턴 종료 후 각 적에 대해 `EnemyAiService`를 실행해 이동/공격을 처리
...
**책임 아님**: FOV 계산, 탐험 상태 저장, 충돌 규칙 정의
